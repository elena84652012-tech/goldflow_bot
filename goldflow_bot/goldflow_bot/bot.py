...
# который слушает какой-то порт, и проверяют это в первые секунды после
# старта процесса. Библиотеки ниже (особенно matplotlib) на слабом бесплатном
# сервере могут загружаться довольно долго, поэтому веб-сервер запускаем
# ПЕРВЫМ делом, до всех тяжёлых импортов — так порт открывается мгновенно,
# и хостинг не успевает решить, что сервис "не поднялся".
...
class _ReusableHTTPServer(HTTPServer):
    allow_reuse_address = True  # позволяет сразу переиспользовать порт при перезапуске


def _start_health_server():
    port = int(os.environ.get("PORT", 10000))
    try:
        server = _ReusableHTTPServer(("0.0.0.0", port), _HealthCheckHandler)
        print(f"Health-check сервер слушает порт {port}", flush=True)
        server.serve_forever()
    except Exception as e:
        print(f"Health-check сервер не смог запуститься: {e}", flush=True)


threading.Thread(target=_start_health_server, daemon=True).start()

# ---------- Основные импорты (могут занимать время на слабом сервере) ----------
